...
const Team = () => {
    ...
    const filterPlayers = (value) => {
        if (team.length === 10 && !teamHasGoalKeeper) {
            const inputFilteredPlayers = initialPlayers.filter(player => player.searchName.toLowerCase().includes(value.toLowerCase()) || player.name.toLowerCase().includes(value.toLowerCase()));
